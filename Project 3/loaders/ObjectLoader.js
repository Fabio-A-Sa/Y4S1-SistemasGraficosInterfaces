...
export const objectLoader = {
  ...
  createPrimitive(primitive, materialId, castShadow =false, receiveShadow=false) {
    ...
    switch (primitive.subtype) {
      ...
      case 'polygon':     
        ...
        break;
      case 'road':
        objPrimitive = this.getRoad(parameters, materialData, material);
        break;
      case 'route':
        objPrimitive = this.getRoute(parameters);
        break;
      default:
        return;
    }
    ...
    while (stack.length > 0) {
      ...
      for(const key in treeNode.children) {
        ...
        // Set animations, if any
          if (treeNode.animation?.length) {
          returnedObject.setAnimationPoints(treeNode.animation);
        }
  ...
  */
  getCylinder: function(parameters, materialData) {
    ...
    objPrimitive.userData.limit = parameters.top;
    return objPrimitive; 
  },
  ...
  */
  getBox: function(parameters, materialData) {
    ...
    objPrimitive.userData.limit = Math.abs(parameters.xyz2[2] - parameters.xyz1[2]) / 2;
  ...
  */
  getSphere: function (parameters, materialData, material) {
    ...
    const objPrimitive = new THREE.Mesh(objGeometry, material);
    objPrimitive.userData.limit = parameters.radius;
  ...
  */
  ...
  /**
   * Generates a road mesh based on the provided parameters and materials.
   * 
   * @param {Object} parameters - Tube parameters.
   * @param {Object} materialData - Material data for the tube.
   * @param {Object} material - Default material data for the tube.
   * @returns {THREE.Mesh} - The generated tube mesh.
  */
  getRoad: function (parameters, materialData, material) {

    // Tube material
    if (materialData) {
      material = materialLoader.create(materialData);
    }

    // Tube control points
    let controlPoints = [];
    for (const controlpoint of parameters.controlpoints) {
      controlPoints.push(new THREE.Vector3(
        controlpoint.xx, controlpoint.yy, controlpoint.zz)
      );
    }
  ...
  /**
   * Generates a path mesh along a Catmull-Rom curve based on the provided control points.
   *
   * @param {Object} parameters - The parameters for creating the path.
   * @param {Array} parameters.controlpoints - An array of control points specifying the path.
   * @returns {THREE.Mesh} - A mesh representing the tube along the Catmull-Rom curve.
   */
  getRoute: function (parameters) {
  
    // Keyframes
    let keyframes = [];
    for (const point of parameters.controlpoints) {
      keyframes.push({
        time: point.time,
        value: new THREE.Vector3(point.xx, point.yy, point.zz)
      });
    }

    // Path
    const path = new THREE.CatmullRomCurve3(keyframes.map(kf => kf.value));
    const tube = new THREE.TubeGeometry(path, 64, 0.4);

    // Invisible or visible material
    const material = new THREE.MeshBasicMaterial({ 
      transparent: !parameters.visible, opacity: 0 
    });
    return new THREE.Mesh(tube, material);
  }
}
