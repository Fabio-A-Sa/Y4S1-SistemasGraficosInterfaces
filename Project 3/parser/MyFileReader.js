...
class MyFileReader  {
	...
	*/
	...
	loadNode(nodeElement) {
		...
		// load animations
		let animation = nodeElement.getElementsByTagName('animation');
		if (animation !== null && animation.length > 0) {
			this.loadAnimation(obj, animation[0]);
		}
	...
	/**
	 * Loads animations
	 * @param {XML element} obj
	 * @param {*} animations the primitive object to load data into
	 * @returns 
	 */
	loadAnimation(obj, animations) {
		const points = [];
		for (const controlpoint of animations.getElementsByTagName('controlpoint')) {
			points.push({
				x: controlpoint.getAttribute('xx'), 
				y: controlpoint.getAttribute('yy'), 
				z: controlpoint.getAttribute('zz'), 
				time: controlpoint.getAttribute('time')
			});
		}
		obj.animation = points;
	}
	...
	loadPrimitive(parentElem, primitiveObj) {
		...
		for(let i=0; i < this.data.primitiveIds.length; i++) {
			...
			for(let j=0; j < elems.length; j++) {
				...
				if (["nurbs", "route", "road"].includes(primitiveId)) this.loadChildElementsOfType(elem, reprObj, "controlpoints", "controlpoint")
